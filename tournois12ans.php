...
/*$selectInsert = "insert into det_tournoi(MATRICULE, DATE_NAISSANCE) select MATRICULE,DATE_NAISSANCE from client";
mysql_query($selectInsert) or (die (mysql_error()));*/


/*$reqTournoi = "insert into det_tournoi (DATE_AUJOURDUI) values('$aujourdui')";
mysql_query($reqTournoi) or (die (mysql_error()));*/


/*$select = "select MATRICULE, DATEDIFF (DATE_NAISSANCE,CURRENT_DATE()) from client";*/
/*$select = "select MATRICULE, date_SUB(DATE_AUJOURDUI, INTERVAL 8 YEAR) as DATE_NAISSANCE from det_tournoi ";
*//*$select = "select MATRICULE from det_tournoi where DATE_NAISSANCE BETWEEN 'DATE_SUB(DATE_AUJOURDUI,INTERVAL 12 YEAR)' and 'DATE_SUB(DATE_AUJOURDUI,INTERVAL 7 YEAR)' ";

*/

/*$reqTournoi1 = "insert into det_tournoi (MATRICULE,DATE_NAISSANCE) values('MATRICULE','') ";
mysql_query($reqTournoi1) or (die (mysql_error()));
*/

/*$select = "select  client.MATRICULE, DATE_NAISSANCE FROM client";
$reqTournoi= "insert into  det_tournoi (DATE_AUJOURDUI,MATRICULE,DATE_NAISSANCE) values ('$aujourdui','6','2005-06-08')" ;
$rSet=mysql_query($reqTournoi) or (die (mysql_error()));*/
/*$select = "select * from client where DATE_NAISSANCE BETWEEN '2002-01-01' and '2014-12-10' ";*/
...
<form method= "POST" action ="ajoutTournois12ans.php" enctype= "multipart/form-data">
	<table >
		<tr>
			...
			<td><a href='tournois12ans.php?id=<?php echo $cl['MATRICULE'];?>'><?php echo ($cl['NOM'])?></a></td>
