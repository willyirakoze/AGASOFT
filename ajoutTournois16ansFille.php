...
/*$tournois="SELECT ID_TOURNOIS FROM tournois WHERE ID_TOURNOIS=1";*/

/*mysql_query($tournois) or (die (mysql_error()));*/
...
$select="SELECT * FROM client WHERE DATE_NAISSANCE BETWEEN '$seize_ans' and '$treize_ans' and SEXE='Feminin' ";
$rSet=mysql_query($select) or (die (mysql_error()));
?>
...
<form method= "POST" action ="ajoutTournois16ansFille.php" enctype= "multipart/form-data">
	<table >
		<tr>
			<td> <?php echo ($cl['MATRICULE'])?>)</td> 
			
			<input type="hidden" name="Rien" value="<?php echo ($cl['MATRICULE'])?>)" />
			<td><a href='tournois16ansFille.php?id=<?php echo $cl['MATRICULE'];?>'><?php echo ($cl['NOM'])?></a></td>
...
<!-- <input type="hidden" name="Rien" value="<?php echo ($cl['MATRICULE'])?>)" /> -->
<input type="number" name="score" value=""  min="0" max="20" onkeypress='only_numeric(event)'placeholder="Note" required autofocus>
