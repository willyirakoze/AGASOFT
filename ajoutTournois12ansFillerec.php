...
$huit_ans = date('Y-m-d',strtotime("-8 years"));
/*$select="SELECT * FROM det_tournoi WHERE DATE_NAISSANCE >=$huit_ans AND DATE_NAISSANCE <=$dix_ans";*/
$select="SELECT * FROM client WHERE DATE_NAISSANCE BETWEEN '$douze_ans' and '$huit_ans'and SEXE='Feminin' ";
$rSet=mysql_query($select) or (die (mysql_error()));
?>
...
<form method= "POST" action ="ajoutTournois12ansFillerec.php" enctype= "multipart/form-data">
	<table >
		<tr>
			<td> <?php echo ($cl['MATRICULE'])?>)</td> 
			
			<input type="hidden" name="Rien" value="<?php echo ($cl['MATRICULE'])?>)" />
			<td><a href='tournois12ansFillerec.php?id=<?php echo $cl['MATRICULE'];?>'><?php echo ($cl['NOM'])?></a></td>
