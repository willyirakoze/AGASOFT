...
$select="SELECT * FROM client WHERE DATE_NAISSANCE BETWEEN '$vingt_ans' and '$dix_sept_ans' and SEXE='Masculin' ";
$rSet=mysql_query($select) or (die (mysql_error()));
?>
...
<form method= "POST" action ="ajoutTournois20ans.php" enctype= "multipart/form-data">
	<table >
		<tr>
			<td> <?php echo ($cl['MATRICULE'])?>)</td> 
			
			<input type="hidden" name="Rien" value="<?php echo ($cl['MATRICULE'])?>)" />
			<td><a href='tournois20ans.php?id=<?php echo $cl['MATRICULE'];?>'><?php echo ($cl['NOM'])?></a></td>
